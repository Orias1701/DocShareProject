...
class Post {
    ...
    // Khởi tạo content để tránh lỗi
    $processedContent = $content;

    if ($fileUrl && $fileType) {
        // Trường hợp 1: Người dùng upload file Word/PDF
        // KHÔNG CẦN trích xuất nội dung và lưu vào cột `content` nữa.
        // Cột `content` sẽ được set thành một giá trị rỗng hoặc null.
        $processedContent = ''; // Hoặc có thể là null, tùy vào cấu trúc database của bạn
    } else {
        // Trường hợp 2: Người dùng nhập nội dung từ editor
        // Xử lý ảnh base64 như bình thường
    ...
    $stmt = $this->pdo->prepare($sql);
    return $stmt->execute([$newId, $title, $processedContent, $albumId, $categoryId, $bannerUrl, $fileUrl, $fileType]);
}
...
public function updatePost($id, $title, $content, $albumId, $categoryId, $bannerUrl, $userId, $fileUrl = null, $fileType = null) {
    ...
    if ($fileUrl && $fileType) {
        // Nếu có file mới, xóa nội dung cũ và lưu thông tin file mới
        $processedContent = ''; // Nội dung sẽ rỗng
    } else {
        // Nếu không có file mới, xử lý nội dung từ editor
        $processedContent = $this->processBase64ImagesInContent($content);
    }
    
    // Thêm `file_url` và `file_type` vào câu lệnh UPDATE
    $sql = "UPDATE posts 
    ...
    return $stmt->execute([$title, $processedContent, $albumId, $categoryId, $bannerUrl, $fileUrl, $fileType, $id, $userId]);
}
