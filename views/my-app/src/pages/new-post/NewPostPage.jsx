...
const NewPostPage = () => {
  ...
  // --- NEW: refs phục vụ hủy AI và chống race-condition ---
  const summarizeCtrlRef = useRef(null);   // giữ AbortController hiện tại
  const summarizeJobIdRef = useRef(0);     // id để bỏ qua response cũ

  const cancelSummarize = (silent = true) => {
    if (summarizeCtrlRef.current) {
      try { summarizeCtrlRef.current.abort(); } catch {}
      summarizeCtrlRef.current = null;
    }
    setIsSummarizing(false);
    // tuỳ chính sách của bạn: giữ hay clear summary khi hủy
    setNewPost((p) => ({ ...p, summary: "" }));
    setIsRejected(false);
    setRejectedMsg("");
    if (!silent) showToast("Đã hủy AI đang phân tích.", "info");
  };
  ...
  useEffect(() => {
    setErrors({});
    if (mode !== "pdf") {
      // chuyển qua WORD: hủy AI nếu còn đang chạy và clear file
      if (mainFile) cancelSummarize(true);
      setMainFile(null);
    }
  ...
  const handleSummarize = async (file) => {
    ...
    // Hủy job cũ (nếu có) trước khi tạo job mới
    if (summarizeCtrlRef.current) {
      try { summarizeCtrlRef.current.abort(); } catch {}
    }

    const ctrl = new AbortController();
    summarizeCtrlRef.current = ctrl;

    // đánh dấu job mới để chống race-condition
    const myJobId = ++summarizeJobIdRef.current;
    ...
    try {
      ...
      // Nếu đã có job khác mới hơn, bỏ qua kết quả này
      if (myJobId !== summarizeJobIdRef.current) return;
      ...
      if (err?.name === "AbortError") {
        // request đã bị hủy do Remove/chọn file mới/đổi mode
        return;
      }
      ...
      // Chỉ tắt trạng thái nếu đây vẫn là job hiện tại
      if (myJobId === summarizeJobIdRef.current) {
        setIsSummarizing(false);
        summarizeCtrlRef.current = null;
      }
    }
  ...
  // Tự map lại category khi list categories đã load xong
  useEffect(() => {
  ...
  // Nếu file bị xóa/Remove từ FilePreview (setMainFile(null)) -> hủy AI
  useEffect(() => {
    if (!mainFile) {
      cancelSummarize(true);
    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <FileUpload
                    ...
                    onFileSelect={(file) => {
                      // Hủy job cũ nếu còn, tránh chạy song song
                      if (summarizeCtrlRef.current) {
                        try { summarizeCtrlRef.current.abort(); } catch {}
                        summarizeCtrlRef.current = null;
                      }
