...
function ProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const meRes = await authApi.me();       // fetchJson trả {status, user}
        const currentUser = meRes?.user;
        const id = currentUser?.user_id;
        if (!id) throw new Error("Không tìm thấy user_id từ api_me");
        setMe(currentUser);

        // 2) Lấy chi tiết từ show_user_info (fetchJson đã unwrap -> { user, isFollowing })
        try {
          const infoRes = await userInfoApi.showUserInfo(id); // -> { user, isFollowing }
          const fromInfo = infoRes?.user ?? null;
          setUserData(fromInfo || currentUser);
        } catch {
          // fallback nếu show_user_info lỗi
  ...
  const handleUpdate = async ({ full_name, bio, birth_date, avatar }) => {
    ...
    // Lấy lại dữ liệu mới (showUserInfo đã fallback nếu route HTML)
    const fresh = await userInfoApi.showUserInfo(me.user_id);
  
    // Chuẩn hoá lấy user object từ các khả năng trả về
    const u =
      fresh?.user ??          // trường hợp BE trả { user: {...} }
      fresh?.data?.user ??    // trường hợp BE trả { data: { user: {...} } }
      fresh?.data ??          // trường hợp BE trả { data: {...user fields...} }
      fresh;                  // raw object
    ...
    <div className="text-white p-4 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
        <div className="lg:col-span-1">
          {/* BioCard nhận object user và tự đọc full_name/bio/birth_date */}
