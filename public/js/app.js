...
function renderGroup1(rows) {
  ...
  rows.forEach(r => {
    html += `<tr>
      <td><div class="post-title" data-postid="${escapeHtml(r.post_id)}">${escapeHtml(r.title)}</div><div class="muted">${escapeHtml(r.excerpt || '')}</div></td>
...
function openPostDetail(postId) {
  ...
  fetchJson(API + '?action=post_detail&post_id=' + encodeURIComponent(postId))
    .then(resp => {
...
function renderPostModal(data) {
  ...
  html += `<div style="margin:10px 0">${escapeHtml(p.content || '')}</div>`;
